/* x402-tip.js v1.0 — 🤙 send pennies of aloha
...
(function(){
  ...
  css.textContent=[
  '#x4-wrap{position:fixed;left:14px;bottom:calc(88px + env(safe-area-inset-bottom,0px));z-index:99997;display:flex;align-items:center;gap:6px;font-family:system-ui,-apple-system,sans-serif}',
  '#x4-pill{background:linear-gradient(90deg,#0052ff,#7c4ba8);color:#fff;border:none;border-radius:999px;padding:10px 16px;font-size:14px;font-weight:600;cursor:pointer;box-shadow:0 4px 18px rgba(0,0,0,.45);white-space:nowrap}',
  '#x4-pill:hover{filter:brightness(1.12)}',
  ...
  '#x4-mini{position:fixed;left:14px;bottom:calc(88px + env(safe-area-inset-bottom,0px));z-index:99997;width:40px;height:40px;border-radius:50%;background:rgba(20,14,30,.9);border:1px solid #7c4ba8;font-size:20px;cursor:pointer;box-shadow:0 4px 14px rgba(0,0,0,.4);display:none;align-items:center;justify-content:center}',
  ...
  '@media (max-width:520px){#x4-pill{font-size:13px;padding:9px 13px}}'
  ].join('\n');
  document.head.appendChild(css);

  var wrap=document.createElement('div'); wrap.id='x4-wrap';
  wrap.innerHTML='<button id="x4-pill">🤙 send pennies of aloha</button><button id="x4-x" title="tuck away">✕</button>';
  var mini=document.createElement('button'); mini.id='x4-mini'; mini.title='send pennies of aloha'; mini.textContent='💸';
